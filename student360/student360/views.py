...
def teacher_dashboard_view(request):
    ...
    if not teacher and not request.user.is_superuser:
        ...
        assignments = TeacherSubjectAssignment.objects.filter(teacher=teacher) if teacher else []
        class_ids = [a.class_obj.id for a in assignments]
        assigned_classes = Class.objects.filter(id__in=class_ids)
...
def student_dashboard_view(request):
    ...
    class_assignments = list(Assignment.objects.filter(class_obj=student.class_obj).select_related('subject'))
